...
print(*task_dim)
print(len(task_dim))
127 127 59 26 16 16 26 59...16 59 26 26 59 59
250
'''
...
'''apply_placement(task_dim, individual.placement)
...
class GAOptimizer(BaseOptimizer):
    ...
    def optimize(self, net_len, task_num,n_devices,task_iner_priority, device_graph, task_unit):
        ...
        127 127 59 26 16 16 26 59...16 59 26 26 59 59
        250 
        Storing number of layers within each task and total task is 250
        '''
        ...
        def initialize(population_size):
            ...
            while len(placements) < population_size:
                ...
                #placements.append(generate_random_placement( [127 16 26 59...16 59], 35, true))
            ...
            '''
            Placement: 11    Mutation rate: 0    Zone mutation rate: 0
            Placement: 18    Mutation rate: 0    Zone mutation rate: 0
            Placement: 1     Mutation rate: 0    Zone mutation rate: 0
            Placement: 5     Mutation rate: 0    Zone mutation rate: 0
            Placement: 9     Mutation rate: 0    Zone mutation rate: 0
            Placement: 19    Mutation rate: 0    Zone mutation rate: 0
            Placement: 21    Mutation rate: 0    Zone mutation rate: 0
            Placement: 15    Mutation rate: 0    Zone mutation rate: 0
            Placement: 0     Mutation rate: 0    Zone mutation rate: 0
        ...
        '''
        '''
        _evaluate(individual, [indivual tasks number of layer], task_iner_priority, device_graph, task_unit,
                             pipeline_batches=self.pipeline_batches=1, batches=self.batches=1,
                             simulator_comm_penalty=self.simulator_comm_penalty=1,
                             simulator_comp_penalty=self.simulator_comp_penalty=1,
                             device_memory_utilization=self.device_memory_utilization=1)
        '''
        ...
        def rank(population, return_scores=False, benchmarking_function=None):
            ...
            # print("genetic_algorithm.py 248")
            # print(fitness_scores)
            # [0.00012290974091164818, 0.00011377850176785507, 0.00012734509616695006, 0.00011452204658760546...] #100 values
            # for i in population:
            #     print(len(i.placement))  Have 12890 total sub task  with placement have their deviceand Mutation_rate=0      Zone_mutation_rate=0
            fitness_db = dict(zip(population, fitness_scores))
            # print("genetic_algorithm.py 246")
            # # print(fitness_db) #{<genetic_algorithm.Candidate object at 0x7fafca84ba60>: 0.0001472235819505702, <genetic_algorithm.Candidate object at 0x7fafcbf2ffa0>: 0.00013957723898928284}
            # for k, v in fitness_db.items():
            #     print(k, v)
            if return_scores:
        ...
        #self.plot_fitness_history is False
        if self.plot_fitness_history:
            fitness_history = []

        #self.print_diversity is False
        ...
        #population_size=10
        #run_optimization(generations=1)
        #generations is number of iteration count, for testing I have entered 5
        def run_optimization(generations, population_size=self.population_size, benchmarking_function=None,
            ...
            # print(len(pop)) #-->100
            '''
            The nonlocal keyword in Python is used to indicate that a variable is not a local variable, but it is not a global variable either. 
            It is used in nested functions where you want to modify a variable from an enclosing (but non-global) scope.
            '''
            for i in tqdm(range(generations), disable=not self.verbose):
                ...
                #Placement: 18    Mutation rate: 0    Zone mutation rate: 0
                '''
                #rank(pop, return_scores=True, benchmarking_function=None)
        ...
        # print("valur of verbose")
        # print(self.verbose)   ---> 5
        if self.verbose:
